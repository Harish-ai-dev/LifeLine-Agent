...
export const WaitlistModal: React.FC<WaitlistModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div className="relative w-full max-w-lg p-6 sm:p-8 rounded-3xl bg-[#0B1120] border border-cyan-800 shadow-2xl text-slate-100">
        ...
        {success ? (
          <div className="text-center py-8 space-y-4 font-mono">
            <div className="w-16 h-16 rounded-2xl bg-emerald-950 border border-emerald-500 mx-auto flex items-center justify-center text-emerald-400">
              <CheckCircle2 className="w-8 h-8" />
            </div>
            <h3 className="text-2xl font-bold text-white">Pilot Registration Confirmed!</h3>
            ...
            <button
              onClick={onClose}
              className="mt-4 px-6 py-2.5 rounded-xl bg-cyan-500 text-slate-950 font-bold text-xs font-mono"
            >
              Done
            </button>
          </div>
          ...
          <div>
            ...
            {error && (
              <div className="p-3 my-3 rounded-lg bg-rose-950 border border-rose-800 text-rose-300 text-xs font-mono">
                {error}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="mt-5 space-y-3 font-mono text-xs">
              ...
              <div>
                <label className="block text-slate-400 mb-1">Work / Professional Email *</label>
                <input
                  type="email"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-400 mb-1">Organization Type</label>
                  <select
                    value={orgType}
              ...
              <div>
                <label className="block text-slate-400 mb-1">Primary Interest Area</label>
                <select
                  value={interestArea}
