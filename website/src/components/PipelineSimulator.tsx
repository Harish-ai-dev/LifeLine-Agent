...
import { CLINICAL_SCENARIOS, PIPELINE_STEPS, ClinicalScenario } from '../data/pipeline';
...
export const PipelineSimulator: React.FC = () => {
  ...
  return (
    <section id="how-it-works" className="py-24 bg-[#0B1120] relative border-t border-slate-900 scroll-mt-20">
      <div id="pipeline" />
      <div id="simulator" />
      
      {/* Background glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[350px] bg-cyan-600/10 blur-[140px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-[11px] text-cyan-400 font-bold">NEWS2: {sc.news2Score}</span>
                </div>
                <div className="font-bold text-xs text-slate-100 truncate">{sc.name}</div>
                <div className="text-[11px] text-slate-400 mt-1">{sc.category}</div>
              </button>
            );
          })}
        </div>
        ...
        <div className="mt-8 max-w-5xl mx-auto p-5 rounded-2xl bg-[#0F172A] border border-cyan-900/60 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-xl bg-cyan-950 border border-cyan-800 flex items-center justify-center text-cyan-400 font-mono font-bold">
              #{activeStepIndex + 1}
            </div>
            <div>
              <div className="text-xs font-mono uppercase tracking-wider text-slate-400">Current Pipeline Stage</div>
              <div className="text-sm font-bold text-white font-mono flex items-center space-x-2">
                ...
                <span className="text-cyan-400 text-xs">({PIPELINE_STEPS[activeStepIndex].durationMs})</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3 w-full md:w-auto">
            ...
            <button
              ...
              className={`px-3 py-2.5 rounded-lg text-xs font-mono border flex items-center space-x-1.5 ${
                showJsonInspector 
        ...
        <div className="mt-8 max-w-5xl mx-auto grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
        ...
        <div className="mt-8 max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
          
          {/* Left 2 Cols: Step Execution Detail */}
          <div className="lg:col-span-2 p-6 rounded-2xl bg-[#0F172A] border border-cyan-950/80 shadow-xl space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-2 pb-4 border-b border-slate-800">
              ...
              <span className="px-2.5 py-1 rounded bg-slate-800 border border-slate-700 text-cyan-300 text-xs font-mono">
            ...
            <div>
              ...
              <div className="space-y-2">
                {PIPELINE_STEPS[activeStepIndex].actionItems.map((action, i) => (
                  <div key={i} className="flex items-center space-x-2.5 text-xs font-mono text-slate-200">
            ...
            <div className="p-4 rounded-xl bg-slate-900/90 border border-cyan-900/40">
              ...
              {activeStepIndex === 1 && (
                <div className="text-xs font-mono text-slate-300 space-y-1.5">
                  <div className="flex items-center space-x-2">
                    <span className="px-2 py-0.5 rounded bg-red-950 text-red-300 font-bold">NEWS2 SCORE = {selectedScenario.news2Score}</span>
                    <span className="text-rose-400 font-bold">ACUITY: {selectedScenario.acuity}</span>
                  </div>
              ...
              {activeStepIndex === 2 && (
                <div className="text-xs font-mono text-slate-300 space-y-1">
                  <p><strong>Matched Facility:</strong> <span className="text-emerald-400 font-bold">{selectedScenario.matchedHospital}</span></p>
              ...
              {activeStepIndex === 3 && (
                <div className="text-xs font-mono text-slate-300 space-y-1">
                  ...
                  <p><strong>Code-3 Emergency ETA:</strong> <span className="text-cyan-400 font-bold">{selectedScenario.travelEta}</span></p>
              ...
              {activeStepIndex === 4 && (
                <div className="text-xs font-mono text-slate-300 space-y-1.5">
                  <p className="text-amber-300 font-bold">Pre-Arrival SBAR Briefing Delivered:</p>
                  <p className="italic text-slate-200">"{selectedScenario.briefingHeadline}"</p>
                </div>
              )}

              {activeStepIndex === 5 && (
                <div className="text-xs font-mono text-slate-300 space-y-1">
                  <p><strong>Cryptographic Audit Hash:</strong> <span className="text-purple-400">{selectedScenario.simulatedAuditHash}</span></p>
          ...
          <div className="p-6 rounded-2xl bg-[#0F172A] border border-slate-800 flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <span className="text-xs font-mono text-slate-400">PATIENT MONITOR</span>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 font-mono text-xs">
                <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
                  ...
                  <div className="text-lg font-bold text-white">{selectedScenario.vitals.hr} <span className="text-xs font-normal text-slate-400">bpm</span></div>
                </div>

                <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
                  <span className="text-[10px] text-slate-400">BLOOD PRESSURE</span>
                  <div className="text-lg font-bold text-white">{selectedScenario.vitals.bp}</div>
                </div>

                <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
                  <span className="text-[10px] text-slate-400">RESP RATE</span>
                  <div className="text-lg font-bold text-white">{selectedScenario.vitals.rr} <span className="text-xs font-normal text-slate-400">/min</span></div>
                </div>

                <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
                  <span className="text-[10px] text-slate-400">SpO2 LEVEL</span>
                  <div className="text-lg font-bold text-cyan-400">{selectedScenario.vitals.spo2}%</div>
                </div>

                <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
                  <span className="text-[10px] text-slate-400">TEMPERATURE</span>
                  <div className="text-lg font-bold text-white">{selectedScenario.vitals.temp}°C</div>
                </div>

                <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
                  <span className="text-[10px] text-slate-400">GCS SCALE</span>
                  <div className="text-lg font-bold text-amber-400">{selectedScenario.vitals.gcs} / 15</div>
                </div>
              </div>

              <div className="mt-4 p-3 rounded-lg bg-slate-900/80 border border-slate-800 text-[11px] font-mono text-slate-300">
                <div className="text-slate-400 mb-1">PRESENTATION NOTES:</div>
            ...
            <div className="mt-6 pt-4 border-t border-slate-800 flex items-center justify-between text-xs font-mono">
              ...
              <span className="text-cyan-400 font-bold">1.78s</span>
            </div>
          </div>
        ...
        {showJsonInspector && (
          <div className="mt-6 max-w-5xl mx-auto p-5 rounded-2xl bg-[#090D16] border border-cyan-800/80 font-mono text-xs">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800 text-cyan-400">
            ...
            <pre className="mt-3 p-4 rounded-xl bg-black/60 text-emerald-400 overflow-x-auto text-[11px] max-h-64 leading-relaxed">
{JSON.stringify({
