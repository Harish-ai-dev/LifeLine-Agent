      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 xl:px-10">
            ...
            return (
              <button
                ...
                className={`p-3.5 rounded-2xl border text-left font-mono transition-all flex flex-col justify-between ${
                  isSelected
                    ? 'bg-cyan-950/80 border-cyan-400 shadow-xl shadow-cyan-500/20 text-white'
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 p-6 sm:p-8 rounded-3xl bg-[#0F172A] border border-cyan-950/80 shadow-2xl space-y-6">
            ...
            <div className="p-4 rounded-xl bg-slate-900/90 border border-slate-800 font-mono text-xs space-y-1.5">
              <div className="text-cyan-400 font-bold uppercase tracking-wider text-[10px]">Clinical Purpose:</div>
              <p className="text-slate-300 leading-relaxed font-sans">{selectedAgent.clinicalPurpose}</p>
            </div>

            {/* Input & Output Plain Language Descriptions */}
            <div className="space-y-3 font-mono text-xs">
              <div className="p-3 rounded-xl bg-slate-900 border border-slate-800">
                <span className="text-slate-400 font-bold block mb-1">RAW INPUT PAYLOAD:</span>
                <span className="text-slate-200 font-sans">{selectedAgent.inputDescription}</span>
              </div>

              <div className="p-3 rounded-xl bg-slate-900 border border-slate-800">
                <span className="text-cyan-400 font-bold block mb-1">DETERMINISTIC OUTPUT:</span>
                <span className="text-slate-200 font-sans">{selectedAgent.outputDescription}</span>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 pb-4 border-b border-slate-800">
                ...
                <button
                  ...
                  className="px-2.5 py-1.5 rounded-lg bg-slate-800 text-xs font-mono text-slate-300 hover:text-white flex items-center space-x-1"
                >
                  {copied ? (
                    <>
              ...
              <div className="mt-4 font-mono text-xs">
                {activeTab === 'INPUT' && (
                  <pre className="p-4 rounded-2xl bg-black/80 text-emerald-400 overflow-x-auto text-[11px] max-h-[460px] leading-relaxed border border-slate-900">
                    {JSON.stringify(selectedAgent.inputSample, null, 2)}
                  </pre>
                )}

                {activeTab === 'OUTPUT' && (
                  <pre className="p-4 rounded-2xl bg-black/80 text-cyan-300 overflow-x-auto text-[11px] max-h-[460px] leading-relaxed border border-slate-900">
                    {JSON.stringify(selectedAgent.outputSample, null, 2)}
                  </pre>
                )}

                {activeTab === 'PROMPT' && (
                  <pre className="p-4 rounded-2xl bg-black/80 text-amber-300 overflow-x-auto text-[11px] max-h-[460px] whitespace-pre-wrap leading-relaxed border border-slate-900">
                    {selectedAgent.promptExcerpt}
                  </pre>
                )}
              </div>
