...
LifeLine Agent is entirely driven by its robust, unified command-line interface. Whether you are running locally or deploying to production, **you never need to manually run `npm install` or `pip install`** — the `lifeline` CLI handles everything.
...
*(If the `lifeline` alias isn't immediately available in your terminal path, you can always run `python -m lifeline [command]` or `./lifeline [command]`.)*
