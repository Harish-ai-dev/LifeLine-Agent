  ...
  const filteredAlerts = hospitalAlerts.filter((alert) => {
    ...
    if (searchQuery.trim()) {
      ...
      return (
        ...
        alert.patient.fullName.toLowerCase().includes(q) ||
    ...
    if (alert.status === 'pending_ack') {
      return (
        <span className="bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-300 border border-red-300 dark:border-red-500/40 px-2.5 py-1 rounded-full text-xs font-mono font-black uppercase tracking-wider flex items-center gap-1.5 animate-pulse">
          ...
          Pending Ack ({alert.tier1SecondsRemaining}s)
        </span>
      );
    }
    ...
    if (alert.status === 'acknowledged') {
      return (
        <span className="bg-sky-100 dark:bg-sky-500/20 text-sky-800 dark:text-sky-300 border border-sky-200 dark:border-sky-500/40 px-2.5 py-1 rounded-full text-xs font-mono font-bold uppercase tracking-wider">
          En Route · {alert.drivingEtaMinutes}m
        </span>
      );
    }
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400 font-mono block">
              ({filteredAlerts.length} Active Cases)
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search case, name, complaint..."
              className="bg-slate-50 dark:bg-[#080d16] border border-slate-200 dark:border-slate-700 rounded-xl pl-9 pr-3 py-1.5 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-sky-500 w-52 sm:w-60 font-mono"
            />
          </div>
      ...
      {filteredAlerts.length === 0 ? (
        ...
        /* ── 7-COLUMN PATIENT ROSTER DATA TABLE WITH HORIZONTAL SCROLL ─────── */
        <div className="bg-white dark:bg-[#0e1424] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
          <div className="px-4 py-2 bg-slate-50 dark:bg-[#080d16] border-b border-slate-200 dark:border-slate-800 flex items-center justify-between text-[11px] font-mono text-slate-500 dark:text-slate-400">
            <span>CLINICAL PATIENT ROSTER (7 DATA COLUMNS)</span>
            <span className="text-[10px] text-sky-600 dark:text-sky-400">Scroll horizontally on mobile →</span>
          </div>

          <div className="overflow-x-auto w-full">
            <table className="w-full text-left text-xs border-collapse min-w-[760px]">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/40 text-[10px] font-mono font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                  <th className="py-3 px-4">Case ID</th>
                  <th className="py-3 px-4">Patient Name</th>
                  <th className="py-3 px-4">Age / Gender</th>
                  <th className="py-3 px-4">Triage / NEWS2</th>
                  <th className="py-3 px-4">Specialty</th>
                  <th className="py-3 px-4">Bay / Bed</th>
                  ...
                  return (
                    <tr
                      ...
                      className={`hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors ${
                        isPending ? 'bg-red-50/40 dark:bg-red-950/20' : ''
                      }`}
                    >
                      {/* 1. Case ID */}
                      ...
                      <td className="py-3 px-4 font-bold text-slate-900 dark:text-white">
                        <div>{alert.patient.fullName}</div>
                        <div className="text-[10px] text-slate-500 dark:text-slate-400 font-normal truncate max-w-[180px]">
                      ...
                      <td className="py-3 px-4 font-mono">
                        ...
                        >
                          NEWS2: {alert.news2Score} ({alert.severity})
                        </span>
                      </td>

                      {/* 5. Specialty */}
                      <td className="py-3 px-4 font-mono text-xs text-purple-700 dark:text-purple-300 font-bold uppercase">
                        {alert.crisisType}
                      </td>

                      {/* 6. Bay / Bed */}
                      <td className="py-3 px-4 font-mono text-slate-800 dark:text-slate-200">
                        <span className="font-bold">{alert.reservedBayId || 'BAY-EM1'}</span>
                        <div className="text-[10px] text-slate-400">ETA {alert.drivingEtaMinutes}m</div>
                      </td>

                      {/* 7. Admission Status & Actions */}
                        ...
                        <div className="flex items-center justify-end gap-1.5 font-mono">
                          ...
                          {isPending && (
                            <button
                              ...
                              className="px-2.5 py-1 bg-red-600 hover:bg-red-500 text-white rounded-lg text-[10px] font-bold shadow-sm"
                            >
                              Ack
                            </button>
                          )}
                          ...
                          <button
                            ...
                            className="px-2.5 py-1 bg-sky-50 hover:bg-sky-100 dark:bg-sky-600/20 dark:hover:bg-sky-600/30 text-sky-700 dark:text-sky-300 border border-sky-200 dark:border-sky-500/40 rounded-lg text-[10px] font-bold flex items-center gap-1"
                          >
                            <span>Dossier</span>
                  ...
                  <div className="space-y-2 flex-1 min-w-0">
                    ...
                    <div>
                      <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                        <span>{alert.patient.fullName} ({alert.patient.age}yo {alert.patient.gender})</span>
