              ...
              >
                <div>
                  ...
                  <span className="text-[10px] text-slate-500 block">
                    {isCritical ? '🚨 Critical Low' : isLow ? '⚠️ Low' : 'Adequate'}
                  </span>
                </div>

                <div className="flex items-center justify-center gap-1 mt-2 pt-2 border-t border-slate-200">
                  <button
                    onClick={() => updateHospitalBloodBank(currentHospital.id, bg, -1)}
                    disabled={count <= 0}
                    className="w-6 h-6 rounded bg-white border border-slate-300 flex items-center justify-center text-slate-700 hover:bg-slate-100 disabled:opacity-30 text-xs"
                  ...
                  <button
                    onClick={() => updateHospitalBloodBank(currentHospital.id, bg, 1)}
                    className="w-6 h-6 rounded bg-white border border-slate-300 flex items-center justify-center text-slate-700 hover:bg-slate-100 text-xs"
      ...
      <div className="pt-2">
        ...
        {hospitalRequests.length === 0 ? (
          <div className="p-8 bg-slate-50 rounded-2xl text-center text-xs text-slate-500 border border-slate-200">
            No active blood or organ requests currently open for this facility.
          </div>
        ) : (
          <div className="space-y-3">
            {hospitalRequests.map((req) => (
              <div
                key={req.id}
                className="bg-slate-50 rounded-2xl p-4 border border-slate-200 space-y-3 text-xs"
              >
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 pb-2">
                  ...
                  <span className="text-slate-500">
                    Fulfilled: <strong className="text-slate-900">{req.unitsFulfilled} / {req.unitsRequested}</strong>
                  </span>
                </div>

                <p className="text-slate-700">
                  <strong>Patient:</strong> {req.patientName} · {req.clinicalIndication}
                </p>
                          ...
                          <div>
                            ...
                            <div className="text-[10px] text-slate-500 mt-0.5 flex items-center gap-1.5">
                              ...
                              <span className="font-mono font-bold text-slate-700">
                                Mode: {donor.travelMode === 'transit' ? '🚆 Transit' : donor.travelMode === 'walking' ? '🚶 Walking' : '🚗 Driving'}
                              </span>
                              <span>·</span>
